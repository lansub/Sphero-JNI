...
void* Sphero::monitorStream(void* sphero_ptr)
{
	...
	for(;;)
	{
		// bRecv contains the number of bytes read. 
		bRecv = recv(_bt_sock, &buf, sizeof(buf), 0);
		if(bRecv <= 0)
		{
			sphero->disconnect();
		...
		{
			dataVec.insert(dataVec.end(), &buf[3], &buf[bRecv]);
			if(SpheroPacket::extractPacket(sphero, &packet_ptr, &dataVec))
			{
...
{
	ssize_t retour;
	if((retour = write(_bt_socket, packet.toByteArray(),  packet.getSize())) <= 0)
	{
		disconnect();
	}
...
void Sphero::rollToPosition(spherocoord_t x, spherocoord_t y,
spherocoord_t aX,spherocoord_t aY, uint8_t initSpeed)
{
...
int Sphero::wake()
{
	...
	if (write(_bt_socket, message1, sizeof(message1)) < 0)
	{
		return -1;
	}
	char message2[10] = { ATT::WRT, handle::TX, 0x00, 0x28, 0x44, 0x00, 0x00, 0x20, 0x00, 0x02 };
	if (write(_bt_socket, message2, sizeof(message2)) < 0)
	{
		return -1;
	}
	char message3[4] = { ATT::WRT, handle::WAKE, 0x00, 0x00 };
	if (write(_bt_socket, message3, sizeof(message3)) < 0)
	{
		return -1;
	}
	return 0;
...
void Sphero::setDataStreaming(uint16_t freq, uint16_t delay, uint32_t mask,
	...
	uint8_t data[dlen];
	data[0] = N >> 8; // Assign 8 MSB of M 
	data[1] = N; // Assign 8 LSB of M
